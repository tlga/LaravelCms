...
/*$(document).on('click', '.postButton', function(e) {
    ...
    $.ajax({
        ...
        data: $(this).parents("form").serialize()+'&content='+content,
        dataType: "json",
        type: "post",
        beforeSend: function() {
        ...
        success: function(data) {
            $('.m-alert').removeClass('show');
            if(data.status === false) {
                swal(
                  'Üzgünüz!',
                  'Hatayla karşılaştık<br />'+data.errors,
                  'error'
                )
            } else {
                swal({
                    title: 'İşlem Başarılı',
                    text: '2 sn sonra yönlendirileceksiniz.',
                    timer: 2000,
                    type: 'success',
                    onOpen: function () {
                        swal.showLoading()
                  ...
                  function (dismiss) {
                      if (dismiss === 'timer') {
                          console.log('2 sn içinde yönlendirilecek!')
                      }
                  }
                )
            }
...
$(document).on('click', '.mediaLibImg', function(e) {
    if($(this).hasClass("mediaLibImgHoverClass")){
        $(this).removeClass('mediaLibImgHoverClass');
        $(this).children('.okIcon').css({'opacity':'0'});
        $('.gallerySettingGroup').html('');

    }else{
        $(this).addClass('mediaLibImgHoverClass');
        $(this).children('.okIcon').css({'opacity':'1'});

    }

        if($('.mediaLibImg').hasClass('mediaLibImgHoverClass')) {
            $('#gallerySettings').find('.gallerySettingGroup').html('<div class="hardRemovedImg btn m-btn--pill m-btn--air btn-outline-danger" url-post="upload/delete/">Kalıcı Olarak Sil</div><div class="galleryContentAdd btn m-btn--pill m-btn--air btn-outline-primary">İçeriğe Ekle</div><div class="featuredImg btn m-btn--pill m-btn--air btn-outline-info">Öne Çıkan Görsel</div>');
            if($('.mediaLibImgHoverClass').length > 1) {
                $('#gallerySettings').find('.gallerySettingGroup').append('<div class="galleryCreateButton btn m-btn--pill m-btn--air btn-outline-success">Galeri Oluştur</div>');
                $('.featuredImg').remove();
            }
...
$(document).on('click', '.hardRemovedImg', function(e) {
    for (var i = 0; i < $('.mediaLibImgHoverClass').length; i++) {
        var originalId = $('.mediaLibImgHoverClass')[i]['attributes']['originalid']['nodeValue'];
        ajaxPost($('.hardRemovedImg').attr('url-post')+originalId,removedImgSuccess);
    }
});

function removedImgSuccess(data) {
    $('.mediaLibImgHoverClass').fadeOut(1000);
}

function galleryElemAdd(data) {
    var mediaLibImg = $('.mediaLibImg');
    var value = $(".mediaLibImg:last-child").attr('fileId');
    if(mediaLibImg.prop('class')) {
        var fileId = value;
        value++;
    } else {
        var fileId = 0;
    ...
    for (var i = 0; i < data.length; i++) {
        say++
        if(data[i].type == 0 ) {
            $('.mediaLib').children('.row').append('<div class="col-md-6 mediaLibImg" fileId="'+ say +'" originalId="'+data[i]["id"]+'" filename="'+data[i]["filename"]+'"><div class="okIcon"></div><img src="/../images/icon_size/'+data[i]["filename"]+'"  class="img-responsive"></div>');
        } else if(data[i].type == 1) {
            $('.mediaLib').children('.row').append('<div class="col-md-6 mediaLibImg" fileId="'+ say +'" originalId="'+data[i]["id"]+'"><div class="okIcon"></div><div class="iframeBlock"></div><iframe src="'+data[i]["filename"]+'" height="160px" width="230px" frameborder="0"></iframe></div>');
        } else {
...
$('#m_modal_gallery').on('show.bs.modal', function () {
    galleryClear();
    activaTab('m_tabs_1_1');
    $('.mediaLib').children('.getButton').css({'display':'none'});
});

function galleryClear(modal) {
    if(modal != 0) {
        $('#m_modal_gallery').modal('hide');
    }
...
$(document).on('click', '.galleryCreateButton', function(e) {

    var html='<div id="links">';

    for (var i = 0; i < $('.mediaLibImgHoverClass').length; i++) {

        var filename = $('.mediaLibImgHoverClass')[i]['attributes']['filename']['nodeValue'];
        html+= '<a href="/images/full_size/'+filename+'"><img src="/images/icon_size/'+filename+'" class="img-fluid galleryThumbImg"></a>';

    }

    $('.note-editable').append(html+"</div>");

    galleryClear();

});

$(document).on('click', '.galleryContentAdd', function(e) {
    for (var i = 0; i < $('.mediaLibImgHoverClass').length; i++) {

        if($('.mediaLibImgHoverClass img')[i]) {
            var sourceText = $('.mediaLibImgHoverClass img')[i]['attributes']['src']['nodeValue'];
            var sourceArray = new Array();
            sourceArray = sourceText.split('/');
            var originalId = $('.mediaLibImgHoverClass')[i]['attributes']['originalid']['nodeValue'] + ',';
            var imgSource = url+'/images/full_size/'+sourceArray[4];
            var createImg = '<img src="'+imgSource+'" originalId="'+originalId+'" class="img-responsive" style="max-width:600px; min-width:200px; height:auto;" width="500px"/>';
            $('.note-editable').append(createImg);
        } else {
            $('.note-editable').append($('.mediaLibImgHoverClass iframe')[0]);
        }
...
function mediaGalleryShow() {
    //if($('#gallerySettings').hasClass('mediaLibImg') == true) {
        var variable = $(".mediaLibImg:last-child").attr('fileId');
        variable++;

    ajaxPost('/admin/getContent/image/'+variable+'/10',galleryElemAdd);
}
$(document).on('click', '.nav-tabs li', function(e) {
    galleryClear(0);

});
$(document).on('click', '.getButton', function(e) {
    $('.mediaLib').children('.getButton').css({'display':'block'});
    mediaGalleryShow();
...
$(document).on('click', '.mediaSelector', function(e) {
    if($(this).index() === 0) {
        $(this).parents('form').attr('action','/upload');
    } else if($(this).index() === 1) {
        $(this).parents('form').attr('action','/upload/video')
    } else {
        $(this).parents('form').attr('action','/upload/foreign')
    }
});

function uploadUrlGet(data) {
    galleryClear(0);
    mediaGalleryShow();
    ...
    activaTab('m_tabs_1_3');
}

$(document).on('click', '.mediaUrlGet', function(e) {

    ajaxPost($(this).parents('form').attr('action'),uploadUrlGet,$(this).parents("form").serialize());
});
...
function deleteCategory(data) {
    ...
    if(data.code === 200) {
        ...
        ajaxPost('admin/getContent/category',getCategory);
    }
...
function confirmButton(selector,url,mediaFn) {

    var selected = $('.confirm').attr('selected-id');
    ...
    $(selector).on('change', function() {
        ...
        $(selector+' option').prop('selected', function() {
    ...
    $(document).on('click','.confirm',function() {
        ...
        for (var i = 0; i < $('#categoryTable').find('.selected').length; i++) {
            $('#tableForm').append('<input type="hidden" name="selectedRows[]" value="'+ $('#categoryTable tr.selected')[i].getAttribute('data-id') +'">')
        }
        ajaxPost(url,mediaFn,$('form#tableForm').serialize());
            ...
            success: function (data) {
                ...
                if(data.code === 400) {
                    swal({
                      ...
                      function (dismiss) {
                          if (dismiss === 'timer') {
                              console.log('3 sn içinde yönlendirilecek!')
                          }
                      }
                    )
                }
                if(data.code === 200) {
                    $('.myAlertStyle').addClass('show');
...
$(document).on('click','.featuredImg', function() {
    var picId = $('.mediaLibImgHoverClass').attr('originalid');
    var pic = $('.mediaLibImgHoverClass').children('img').attr('src');
    $('#m-dropzone-one').html('<img src="'+pic+'" class="img-responsive">');
    $('#m-dropzone-one').append('<input type="hidden" name="featured" value="'+picId+'"/>');
    $('#m-dropzone-one').append('<div class="featuredRemove" style="cursor:pointer;">Öne Çıkan Görseli Kaldır</div>');
    galleryClear();
    mediaGalleryShow();
});

$(document).on('click','.featuredRemove', function() {
    var picId = $('.mediaLibImgHoverClass').attr('originalid');
    var pic = $('.mediaLibImgHoverClass').children('img').attr('src');
    $('#m-dropzone-one').html('<div class="m-dropzone dz-clickable" id="m-dropzone-one"> <div class="m-dropzone__msg dz-message needsclick"> <h3 class="m-dropzone__msg-title">Yüklemek İçin Buraya Tıkla</h3> <span class="m-dropzone__msg-desc">Kapak için tek bir .jpg, .jpeg veya .png dosyası seçiniz! max: 8 mb </span> </div> </div>');
});

$(document).on('click','.mediaLibButton', function() {
    $('#m_modal_gallery').modal('show');
});