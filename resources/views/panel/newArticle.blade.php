...
@endsection
@section('content')
	<form method="post" name="pageForm" action="@if(isset($post_title)) {{ route('admin.articleEdit') }} @else {{ route('admin.newArticleSave') }} @endif">
	<div class="m-content">
		...
		<div class="row">
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
					...
					<div class="m-form m-form--fit m-form--label-align-right">
						...
						<div class="m-portlet__foot m-portlet__foot--fit">
							<div class="m-form__actions m-form__actions">
								<div class="row">
									<div class="col-lg-6">
										<a href="@if(isset($post_id)){{ route("admin.articleDelete")."/".$post_id }}@else {{ url()->current() }} @endif"  class="btn m-btn--pill m-btn--air btn-outline-danger">
											Çöp
										</a>
										</div>
									<div class="col-lg-6">
				...
				<div class="m-portlet">
					...
					<!--begin::Form-->
					<div class="m-form m-form--fit m-form--label-align-right">
						<div class="m--padding-10 m-checkbox-list">
							<div class="m-scrollable" data-scrollable="true" data-max-height="200">
								<div class="checkbox">
  								 @foreach($kategoriler as $kategori)
										 <div class="form-group">
												<label><input type="checkbox" name="kategori[]" value=" {{ $kategori->id }}" @if(isset($selectedCategory)) @foreach($selectedCategory as $selectedCategories) @if($selectedCategories->term_taxonomy_id == $kategori->id) checked @endif @endforeach @endif> {{ $kategori->name }}</label>
										 </div>
										 @endforeach
								</div>
							</div>
				...
				<div class="m-portlet">
					...
					<!--begin::Form-->
					<div class="m-form m-form--fit m-form--label-align-right" id="tagForm">
						<div class="m--padding-10 m-checkbox-list">
								<div class="input-group">
									<input type="text" class="form-control" name="name" placeholder="Etiket Yaz...">
										<span class="input-group-btn">
											<button class="btn btn-secondary tagSave" type="button">
												Ekle
											</button>
										</span>
								</div>
						...
						@if(!empty(isset($tag)))
							@foreach($tag as $tags)
							<div data-id="{{ $tags->id }}" class="btn btn-info miniTag">{{ $tags->name }}<input type="checkbox" class="hidden" name="etiket[]" value="{{ $tags->id }}" checked/></div>
							@endforeach
						@endif
					</div>
...
@section('Footer_Script')
	...
	$(document).ready(function(){
			...
			@if(isset($post_content))
				var markupStr = '<?php print_r($post_content) ?>';
				$('.summernote').summernote('code', markupStr);
			@endif

		$('input[name=name]').keypress(function (e) {
			if (e.which == 13) {
				ajaxPost('{{ URL::Route('admin.saveTags') }}',postAction,$('input[name=name]'));
	...
	function fileuploadSuccessFn(file,done) {
		...
		function postAction(data) {
			...
			$('#tagForm').append('<div data-id="'+tagid+'" class="btn btn-info miniTag">'+tagname+'<input type="checkbox" class="hidden" name="etiket[]" value="'+tagid+'" checked/></div>');
		}
